...
test('child injecting', () => {
    ...
    const instance = ObjectManager.getInstance(Child);

    expect(instance.sameFooService).toBeDefined();
    expect(instance.sameFooService).toBeInstanceOf(Foo);
    expect(instance.sameFooService.checker).toEqual(fooService.checker);
    expect(instance.sameFooService.something).toEqual(fooService.something);

    expect(instance.otherFooSample).toBeDefined();
    expect(instance.otherFooSample).toBeInstanceOf(Foo);
